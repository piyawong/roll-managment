...
for (let clientId = 2; clientId <= 10; clientId++) {
  ...
  try {
    ...
    // Find the position to insert preload (after selectedCompletedImageIndex useEffect)
    const insertAfter = '}, [selectedCompletedImageIndex, selectedCompletedFolder]);';
  ...
  useEffect(() => {
    ...
    const newContent = content.substring(0, insertPos + insertAfter.length) + preloadCode + content.substring(insertPos + insertAfter.length);
